...
In order: show 21 days, starting from the present day, not highlighting the weekends, don't warn about deadlines beyond today in today's agenda, and show done tasks (in a "log mode" view, but not in the "list" view).
