...
(defun ning/sync ()
  ...
  (save-excursion
    ...
    (make-process
     :name "ning/sync"
     :buffer "*unison*"
     :command `("unison" "-batch" "-ignore" "Name {.notdeft-db,elfeed-db}" ,(expand-file-name "~/roamdb") "ssh://meowmeow//home/admin/roamdb")
...
(defun ning/sync-synchronous ()
  ...
  (let ((coding-system-for-read 'undecided-mac))
    (call-process "unison" nil "*unison*" t "-ignore" "Name {.notdeft-db,elfeed-db}" "-batch" (expand-file-name "~/roamdb") "ssh://meowmeow//home/admin/roamdb")))
#+end_src
...
(map! :map 'override "<next>" #'ning/next-heading-for-projector)
(map! :map 'override "<prior>" #'ning/prev-heading-for-projector)
#+end_src
