...
(defun ning/sync ()
  "Sync the ~/org directory to ssh:meowmeow"
  (interactive)
  (set-process-sentinel
   (start-process "Sync with unison" "*unison*"
                  "unison" "-batch" (expand-file-name "~/org/") "ssh://meowmeow//org")
   (ning/get-unison-sentinel t)))

(defun ning/sync-synchronous ()
  "Sync the ~/org directory to ssh:meowmeow in the foreground"
  (interactive)
  (delete-other-windows)
  (switch-to-buffer "*unison*")
  (call-process "unison" nil "*unison*" t
                "-batch" (expand-file-name "~/org") "ssh://meowmeow//org"))
#+end_src
...
* Start-up, idle, and exit commands
:PROPERTIES:
...
** Start-up
:PROPERTIES:
:ID:       14975582-b8fd-49c8-961f-f8424d2c722a
:END:
...
** Idle
:PROPERTIES:
:ID:       abb2b332-c217-4cbe-8304-957269954b79
:END:

#+begin_src emacs-lisp
(run-with-idle-timer 60 nil 'ning/sync)
#+end_src
...
#+begin_src emacs-lisp
(add-hook 'kill-emacs-hook 'ning/sync-synchronous)
#+end_src
