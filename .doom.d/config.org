...
(use-package! org-tempo
  ...
  (tempo-define-template "from-pubmed"
                         '("Discovered via PubMed search alert for ~(Singapore[Affiliation]) AND (Cancer[Text Word])~.")
                         "<p" "Describe discovery via PubMed search alert"))
#+end_src
...
(defun ning/on-termux-p ()
  (if (string-match-p "com.termux" (getenv "PREFIX")) t nil))
